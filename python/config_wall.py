...
def configSkydiveLink(ccvm):
    ...
    link_update_query = "UPDATE dashboard SET data = REPLACE(data, SUBSTRING_INDEX(SUBSTRING_INDEX(data, 'http://', -1), ':8082', 1), str(ccvm[0])) WHERE org_id = 1 AND folder_id = 0 AND is_folder = 0 AND data LIKE '%http://%:8082%';"
...
def configMoldUserDashboard():
    ...
    user_dashboard_query = "SELECT uid, slug FROM dashboard WHERE title = '가상머신 상세 현황' AND org_id = 2"
